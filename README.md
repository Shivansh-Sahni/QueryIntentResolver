...
The active implementation is under [`v1/`](./v1/). The complete repository map is in [`PROJECT_INDEX.md`](./PROJECT_INDEX.md), and the live project state is in [`v1/CURRENT_STATUS.md`](./v1/CURRENT_STATUS.md).
...
- completed identical-benchmark evaluation of Anthony's Qwen classifier;
- optional zero-shot and API-language-model evaluation paths;
...
- automated GitHub Actions build, testing, release packaging, artifact publication, and generated-artifact commits.
...
- latest automated GitHub Actions build: **passed**, including Qwen scoring, release regeneration, artifact upload, and generated-artifact commit.

The current safety-focused supervised recommendation remains provisional:

- Accuracy: **78.0%**
- Macro F1: **0.7784**
- False short-circuit rate: **4.76%**
- Short-circuit recall: **53.33%**
- P95 local inference latency: approximately **4.26 ms/query**
- Marginal local inference API cost: **$0**

The completed Qwen transfer comparison achieved **27.33% accuracy** and **0.1771 macro F1** with approximately **4.15 seconds P95 latency**. It was scored exactly as produced by the unchanged inference pipeline; no post-hoc intent reparsing or benchmark-driven tuning was applied. The current persona/intent Qwen model is therefore not competitive for the four-route V1 task.

A diagnostic LinearSVC variant reached **84.0% accuracy** and **0.8402 macro F1**, but its **15.28% false short-circuit rate** violates the routing-safety ceiling. Diagnostic models are explicitly blocked from recommendation and packaging.

The recommendation remains provisional because the current release model does not yet meet the frozen minimum short-circuit recall, the 55 unresolved labels remain under review, and the system has not yet been validated on privacy-safe real MascotGO traffic.

### Start here

1. [Current status and active ownership](./v1/CURRENT_STATUS.md)
2. [Complete project index](./PROJECT_INDEX.md)
...
- **Part 1 - Phases 6 and 7:** original complexity-first modeling experiments, contributor datasets, and the first label-ambiguity audit.
- **Part 2 - Step 3:** earlier standalone persona/intent validation package and reporting workflow.
- **research_archive/2026-08-30-hardcore-v1:** separate hybrid-router experiment preserved with its distinct dataset, benchmark, metrics, and limitations.
